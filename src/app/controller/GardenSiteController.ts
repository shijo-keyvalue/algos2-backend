...
import { GardenSiteService } from "../service/GardenSiteService"; // Import the GardenSiteService
import APP_CONSTANTS from "../constants";

class GardenSiteController extends AbstractController {
  private gardenSiteService: GardenSiteService; // Add the GardenSiteService

  constructor(gardenSiteService: GardenSiteService) {
    super(`${APP_CONSTANTS.apiPrefix}/garden-sites`);
    this.gardenSiteService = gardenSiteService;
  ...
  protected initializeRoutes = (): void => {
    this.router.get(
      `${this.path}`,
      this.asyncRouteHandler(this.getAllGardenSites)
    );
    this.router.get(
      `${this.path}/:id`,
  ...
  private getAllGardenSites = async (
    request: RequestWithUser,
    response: Response,
    next: NextFunction
  ) => {
    const getGardenSitesParams: any = request.query;
    const { data, total } = await this.gardenSiteService.getAllGardenSites(getGardenSitesParams);
...
export default GardenSiteController;
