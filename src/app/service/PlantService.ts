...
export class PlantService {
  constructor(
    private plantDao: PlantDao, // Use the appropriate repository or DAO
  ) {}
...
//   }
  ...
  public getAllPlants = async (params: any): Promise<SearchResult> => {
    const { data, total } = await this.plantDao.getAllPlants(params);
    return {
      data: data,
      length: data.length,
      total,
